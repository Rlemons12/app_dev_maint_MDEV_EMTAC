...
ai_layer/providers/embeddings/settings.py Embedding provider settings
ai_layer/providers/embeddings/client.py   Embedding provider client
...
ai_layer/providers/hf_router_client.py    Hugging Face Router helper client
```
...
Embedding and Hugging Face dependencies are already included in `requirements.txt`.
...
Ask with Hugging Face Router/Qwen instead of the default OpenAI provider:
...
`use_tools=False` is recommended for simple text review tasks that do not need PostgreSQL MCP tool calls.
The Hugging Face token must be allowed to call Inference Providers through Hugging Face Router.
...
```powershell
python -m ai_layer.cli.chat_cli
```
...
```text
ai_layer/providers/hf_router_client.py
```
...
```powershell
python -c "from ai_layer.providers.hf_router_client import describe_image_url; print(describe_image_url('https://cdn.britannica.com/61/93061-050-99147DCE/Statue-of-Liberty-Island-New-York-Bay.jpg'))"
```
