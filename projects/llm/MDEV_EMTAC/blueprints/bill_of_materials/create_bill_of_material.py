#blueprints/bill_of_materials/create_bill_of_material.py
...
def create_bill_of_material():
    ...
    # --------------------------------------------------
    # GET → return full page
    # --------------------------------------------------
    if request.method == 'GET':
        logger.info("GET request - rendering full BOM page")
    ...
    logger.info(f"POST request | is_ajax={is_ajax}")
    ...
    try:
        ...
        logger.debug(
            f"Form data: area={area_id}, "
            f"eq_group={equipment_group_id}, "
            f"model={model_id}, "
            f"asset={asset_number_id}, "
        ...
        logger.info(f"Positions found: {len(position_ids)}")
        ...
        # -----------------------------
        # CLEAR OLD RESULTS
        # IMPORTANT: use current session
        # -----------------------------
        clear_bom_results(db_session)

        # -----------------------------
        # BUILD RESULTS
        # -----------------------------
        for position_id in position_ids:
            logger.debug(f"Processing position_id={position_id}")

            associations = (
                db_session.query(PartsPositionImageAssociation)
                .filter_by(position_id=position_id)
                .all()
            )

            logger.debug(
                f"Found {len(associations)} association(s) for position_id={position_id}"
            )

            for assoc in associations:
                part = db_session.query(Part).filter_by(id=assoc.part_id).first()
        ...
        db_session.commit()
        logger.info("BOM results committed")

        # -----------------------------
        # FETCH RESULTS (FOR DISPLAY)
        # -----------------------------
        results = db_session.query(BOMResult).order_by(BOMResult.id.desc()).all()

        parts_and_images = []
        for r in results:
            part = db_session.query(Part).filter_by(id=r.part_id).first()
            image = None

            if r.image_id:
                image = db_session.query(Image).filter_by(id=r.image_id).first()
...
def clear_bom_results(session=None):
