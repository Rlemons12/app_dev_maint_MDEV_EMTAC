...
def initialize_fts_table():
    ...
    try:
        ...
        error_id(f"Error initializing FTS table: {e}", exc_info=True)
...
def add_document():
    ...
    info_id(
        "[DOCUMENT-UPLOAD] Received request to add document(s)",
        request_id,
    )

    if "files" not in request.files:
        error_id("[DOCUMENT-UPLOAD] No files uploaded", request_id)
        return jsonify({"message": "No files uploaded"}), 400

    files = request.files.getlist("files")

    if not files or all(not getattr(f, "filename", "").strip() for f in files):
        error_id("[DOCUMENT-UPLOAD] No valid files provided", request_id)
    ...
    info_id(
        f"[DOCUMENT-UPLOAD] Processing {len(files)} file(s): "
        f"{[getattr(f, 'filename', '') for f in files]}",
        request_id,
    )
    debug_id(f"[DOCUMENT-UPLOAD] Metadata: {metadata}", request_id)

    try:
        _ensure_fts_table_exists(request_id)

        with log_timed_operation(
            "Document processing with FileProcessingCoordinator",
            request_id,
        ):
            warning_id(
                "[TRACE-DOC-UPLOAD] /documents/add_document route calling "
                "FileProcessingCoordinator.process_upload",
                request_id,
            )

            success, response, status = FileProcessingCoordinator().process_upload(
                files=files,
                metadata=metadata,
                concurrent=False,
                max_workers=4,
        ...
        if success:
            info_id(
                "[DOCUMENT-UPLOAD] Successfully processed uploaded document(s)",
                request_id,
            )
        ...
        error_id(
            f"[DOCUMENT-UPLOAD] Processing failed: {response}",
            request_id,
        )
        _log_failed_processing(files, response, request_id)
        return jsonify(response), status

    except Exception as e:
        error_id(
            f"[DOCUMENT-UPLOAD] Error processing files: {e}",
            request_id,
            exc_info=True,
        )
    ...
    try:
        add_audit_log_entry(
            ...
            new_data={
                ...
                "processing_method": "file_processing_coordinator",
                "request_id": request_id,
            },
        )
        commit_audit_logs()
...
def serve_image(image_id):
    ...
    try:
        success, response, status_code = Image.serve_file(
            image_id,
            request_id,
        )
...
def _ensure_fts_table_exists(request_id):
    try:
        ...
        warning_id(
            f"Error checking/creating FTS table: {e}",
            request_id,
        )
...
def initialize_fts():
    ...
    try:
        ...
        if success:
            info_id(
                "FTS table initialized successfully via admin endpoint",
                request_id,
            )
            return jsonify(
                {
                    "message": "FTS table initialized successfully",
                }
            ), 200

        error_id(
            "FTS table initialization failed via admin endpoint",
            request_id,
        )
        return jsonify(
            {
                "message": "FTS table initialization failed",
            }
        ), 500

    except Exception as e:
        error_id(
            f"Error in manual FTS initialization: {e}",
            request_id,
            exc_info=True,
        )
        return jsonify(
            {
                "message": f"Error initializing FTS table: {str(e)}",
            }
        ), 500
...
@add_document_bp.route(
    "/document/<int:document_id>/images_with_chunks",
    methods=["GET"],
)
@with_request_id
def get_document_images_with_chunks(document_id):
    ...
    try:
        images_with_context = CompleteDocument.get_images_with_chunk_context(
            document_id,
            request_id,
        )
        ...
        error_id(
            f"Error getting images with chunks for document {document_id}: {e}",
            request_id,
            exc_info=True,
        )
        return jsonify(
...
@add_document_bp.route(
    "/document/<int:document_id>/visual_summary",
    methods=["GET"],
)
@with_request_id
def get_document_visual_summary(document_id):
    ...
    try:
        visual_summary = CompleteDocument.get_document_visual_summary(
            document_id,
            request_id,
        )
        ...
        error_id(
            f"Error getting visual summary for document {document_id}: {e}",
            request_id,
            exc_info=True,
        )
        return jsonify(
...
def search_images_by_text():
    ...
    try:
        ...
        error_id(
            f"Error searching images by text '{search_text}': {e}",
            request_id,
            exc_info=True,
        )
        return jsonify(
...
def calculate_optimal_workers(
    ...
    memory_per_thread = 100 * 1024 * 1024
...
def add_document_to_db_multithread(*args, **kwargs):
    warning_id(
        "add_document_to_db_multithread is deprecated; "
        "use FileProcessingCoordinator.process_upload()"
    )
    return None, False