# Main Tables with Campus and Building Integration
...
class Building(Base):
    ...
    def find_related_entities(cls, session, identifier, is_id=True, request_id=None):
        ...
        downward = {
            'site_locations': building.site_locations,
...
# Classes dealing with machine subassemblies
...
class Subassembly(Base):
    ...
    def add_subassembly(cls, session, name, location_id, asset_number_id=None, description=None, request_id=None):
        ...
        new_sub = cls(name=name, location_id=location_id, asset_number_id=asset_number_id, description=description)
        session.add(new_sub)
        session.commit()
    ...
    def search(cls, session, name=None, location_id=None, asset_number_id=None):
        """Search Subassemblies by name, location_id, or asset_number_id."""
        query = session.query(cls)
        if name:
            query = query.filter(cls.name.ilike(f"%{name}%"))
        if location_id:
    ...
    def find_or_create(cls, session, name, location_id, asset_number_id=None, description=None, request_id=None):
        ...
        if sub:
            return sub
        sub = cls(name=name, location_id=location_id, asset_number_id=asset_number_id, description=description)
        session.add(sub)
        session.commit()
        return sub
    ...
    def find_related_entities(cls, session, subassembly_id, request_id=None):
        """Traverse relationships: upward (location, asset), downward (component assemblies, positions)."""
        sub = session.query(cls).filter_by(id=subassembly_id).first()
        if not sub:
            return None
        return {
...
class ComponentAssembly(Base):
    ...
    def add_component(cls, session, name, subassembly_id, asset_number_id=None, description=None, request_id=None):
        ...
        comp = cls(name=name, subassembly_id=subassembly_id, asset_number_id=asset_number_id, description=description)
        session.add(comp)
        session.commit()
    ...
    def search(cls, session, name=None, subassembly_id=None, asset_number_id=None):
        """Search ComponentAssemblies by name, subassembly_id, or asset_number_id."""
        query = session.query(cls)
        if name:
            query = query.filter(cls.name.ilike(f"%{name}%"))
        if subassembly_id:
    ...
    def find_or_create(cls, session, name, subassembly_id, asset_number_id=None, description=None, request_id=None):
        ...
        filters = {'name': name, 'subassembly_id': subassembly_id}
        if asset_number_id is not None:
            filters['asset_number_id'] = asset_number_id
    ...
    def find_related_entities(cls, session, component_id, request_id=None):
        """Traverse relationships: upward (subassembly, asset), downward (assembly views, positions)."""
        comp = session.query(cls).filter_by(id=component_id).first()
        if not comp:
            return None
        return {
...
class AssemblyView(Base):  # TODO rename to ComponentView
    ...
    @classmethod
    @with_request_id
    def add_view(cls, session, name, component_assembly_id, asset_number_id=None, description=None, request_id=None):
        """Add a new AssemblyView (ComponentView)."""
        view = cls(name=name, component_assembly_id=component_assembly_id, asset_number_id=asset_number_id,
                   description=description)
        session.add(view)
        session.commit()
    ...
    @classmethod
    @with_request_id
    def find_or_create(cls, session, name, component_assembly_id, asset_number_id=None, description=None,
                       request_id=None):
        """Find an AssemblyView by name + component_assembly + asset, or create it."""
        filters = {'name': name, 'component_assembly_id': component_assembly_id}
        if asset_number_id is not None:
            filters['asset_number_id'] = asset_number_id
    ...
    def find_related_entities(cls, session, view_id, request_id=None):
        """Traverse relationships: upward (component assembly, asset), downward (positions)."""
        view = session.query(cls).filter_by(id=view_id).first()
        if not view:
            return None
        return {
            ...
            try:
                ...
                sql = text("""
                           ...
                           VALUES (:title, :content,
                                   to_tsvector('english', :title || ' ' || :content)) ON CONFLICT (title) DO
                           UPDATE SET
        ...
        with db_config.main_session() as session:
            try:
                ...
                sql = text("""
                           ...
                           VALUES (:area_id, :equipment_group_id, :model_id, :asset_number_id,
        ...
        try:
            ...
            similarity_query = text("""
                      ...
                      de.created_at
                                    FROM complete_document cd
                                        JOIN document d
                                    ON cd.id = d.complete_document_id
...
class Problem(Base):
    ...
    @classmethod
    @with_request_id
    def update_problem(cls, session, problem_id: int, name: str = None, description: str = None,
