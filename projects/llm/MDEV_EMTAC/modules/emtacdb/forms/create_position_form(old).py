...
def cnp_form_create_position(
    ...
    All IDs are optional; this supports "minimal position" creation (e.g., only Area/EquipmentGroup)
    while still allowing deep, fully-specified hierarchical positions.

    If a Position with the same combination already exists, its ID is returned instead
    of creating a duplicate.
    """
    try:
        logger.debug(
            (
                "Entering cnp_form_create_position with IDs: "
                "Area=%s, EquipmentGroup=%s, Model=%s, AssetNumber=%s, "
                "Location=%s, SiteLocation=%s, Subassembly=%s, "
                "ComponentAssembly=%s, AssemblyView=%s"
            ),
            area_id,
            equipment_group_id,
            model_id,
        ...
        area_entity = (
            session.query(Area).filter_by(id=area_id).first() if area_id else None
        )
        equipment_group_entity = (
        ...
        model_entity = (
            session.query(Model).filter_by(id=model_id).first() if model_id else None
        )
        asset_number_entity = (
            session.query(AssetNumber)
            .filter_by(id=asset_number_id)
            .first()
            if asset_number_id
            else None
        )
        location_entity = (
            session.query(Location).filter_by(id=location_id).first()
            if location_id
            else None
        )
        site_location_entity = (
            session.query(SiteLocation)
            .filter_by(id=site_location_id)
            .first()
            if site_location_id
            else None
        )
        subassembly_entity = (
            ...
            .filter_by(id=subassembly_id)
            .first()
            if subassembly_id
            else None
        )
        component_assembly_entity = (
            session.query(ComponentAssembly)
            .filter_by(id=component_assembly_id)
            .first()
        ...
        if existing_position:
            logger.debug(
                "Found existing Position with ID: %s", existing_position.id
            )
        ...
        logger.error(
            "Error in cnp_form_create_position: %s", str(e), exc_info=True
        )
        session.rollback()
        raise e
...
class CreatePositionForm(FlaskForm):
    ...
    hierarchy nodes (Area, EquipmentGroup, Model, AssetNumber, Location,
    Subassembly, ComponentAssembly, AssemblyView, SiteLocation).

    Design goals:
    - Allow minimal positions (e.g., only Area + EquipmentGroup).
    ...
    def save(self, session, cnp_form_create_position):
        ...
        # --------------------------------------------------------------
        # 2. EQUIPMENT GROUP (linked to Area via area_id)
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # 3. MODEL (optionally linked to EquipmentGroup)
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # 4. ASSET NUMBER (linked to Model)
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # 5. LOCATION (linked to Model, optionally asset-specific)
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # 6. SUBASSEMBLY (linked to Location, optionally asset-specific)
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # 7. COMPONENT SUBASSEMBLY (linked to Subassembly, optionally asset-specific)
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # 8. SUBASSEMBLY VIEW (linked to Component Subassembly, optionally asset-specific)
        # --------------------------------------------------------------
        ...
        if self.site_location_input.data:
            logger.debug(
                "New Site Location input provided: %s",
                self.site_location_input.data,
            )
        ...
        logger.debug(
            (
                "Final IDs - Area: %s, EquipmentGroup: %s, Model: %s, "
                "AssetNumber: %s, Location: %s, SiteLocation: %s, "
                "Subassembly: %s, ComponentSubassembly: %s, SubassemblyView: %s"
            ),
            area_id,
            equipment_group_id,
            model_id,
        ...
        # --------------------------------------------------------------
        # 10. Create / reuse Position using helper
        # --------------------------------------------------------------
