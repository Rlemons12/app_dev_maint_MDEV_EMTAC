"""
Gemma 2-2B-Instruct MLflow Smoke Test (Offline, DB-Free)
Verifies MLflow logging, offline model/tokenizer load, and Trainer integration.
"""
...
# ---------------------------------------------------------------------
MODEL_PATH = os.getenv("MODELS_GEMMA_DIR")
LOGS_DIR = os.getenv("SMOKE_TESTS")

if not MODEL_PATH or not os.path.isdir(MODEL_PATH):
    raise FileNotFoundError(
        f"Gemma model directory not found:\n{MODEL_PATH}\n"
        "Please verify MODELS_GEMMA_DIR in your .env file."
    )
...
tmp_dir = os.path.join(LOGS_DIR, "tmp_gemma")
os.makedirs(tmp_dir, exist_ok=True)
os.environ["TMPDIR"] = tmp_dir
os.environ["TEMP"] = tmp_dir

results_dir = os.path.join(LOGS_DIR, "results_gemma")
os.makedirs(results_dir, exist_ok=True)

mlflow.set_tracking_uri(f"file:///{mlruns_path}")
mlflow.set_experiment("Gemma2-2B-SmokeTest")
...
data = {
    "text": [
        "Gemma 2-2B is a compact instruction-tuned model by Google.",
        "This smoke test checks offline training and MLflow logging.",
        "We are verifying dataset tokenization and Trainer evaluation.",
    ]
}
...
passed = False
try:
    with mlflow.start_run(run_name="Gemma2-2B_SmokeTest_Run"):
        ...
        print("Starting Gemma 2-2B-Instruct smoke test (2 steps)...")
        trainer.train()
        ...
        test_save_dir = os.path.join(results_dir, "gemma_smoke_artifact")
...
if passed:
    print("\n==============================")
    print("GEMMA 2-2B SMOKE TEST PASSED")
    print("==============================")
    sys.exit(0)
else:
    print("\n==============================")
    print("GEMMA 2-2B SMOKE TEST FAILED")
    print("==============================")
    sys.exit(1)
