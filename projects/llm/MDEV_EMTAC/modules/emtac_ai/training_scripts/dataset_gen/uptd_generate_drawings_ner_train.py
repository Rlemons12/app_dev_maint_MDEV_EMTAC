...
# IMPORTANT:
# t_training_data MUST be bound to schema="intent_training"
# (as already done in your init_db / metadata setup)

def upsert_intents_into_training_data(
    session: Session,
    intent_samples: List[str],
    intent_name: str = "drawings",
) -> int:
    """
    Insert intent texts into intent_training.training_data with data_type='intent'.

    De-duplicates via UNIQUE (data_type, data_content, label)
    ...
    # ------------------------------------------------------------------
    # Ensure unique constraint/index exists (correct schema)
    # ------------------------------------------------------------------
