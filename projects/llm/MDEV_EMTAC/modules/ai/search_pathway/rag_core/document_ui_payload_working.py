...
class DocumentUIPayload:
    ...
    def enrich_with_parts(
        ...
        for doc in self._documents.values():
            ...
            # ----------------------------------
            # 🔑 Resolve drawing numbers (ONCE)
            # ----------------------------------
            drawings_by_part = drawing_part_service.get_drawing_numbers_by_part_ids(
                part_ids=part_ids,
                session=session,
            )
            ...
            for part in parts_by_id.values():
                ...
                serialized["drawing_numbers"] = drawings_by_part.get(part.id, [])
                doc["parts"].append(serialized)
                debug_id(
                    f"[PART SERIALIZED] part_id={part.id} drawings={serialized['drawing_numbers']}",
                    request_id,
                )
