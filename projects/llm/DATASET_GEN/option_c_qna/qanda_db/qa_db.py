...
class Document(Base):
    ...
    @classmethod
    def create(cls, session, file_name: str, file_path: str):
        doc = cls(file_name=file_name, file_path=file_path)
        session.add(doc)
        session.flush()
        return doc
...
class Chunk(Base):
    ...
    id = Column(Integer, primary_key=True)
    document_id = Column(Integer, ForeignKey("qna_documents.id"), nullable=False)
...
class LLMModel(Base):
    ...
    # Example: "option_c_qna.models.qwen.QwenAnswerGenerator"
...
class Embedding(Base):
    ...
    parent_type = Column(Enum(*PARENT_TYPES, name="parent_type_enum"), nullable=False)
    ...
    meta_json = Column(JSONB, nullable=True, default={})

    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    # Each embedding is produced in exactly one run
    run_item = relationship(
        "PipelineRunItem",
        uselist=False,
        primaryjoin="and_(foreign(PipelineRunItem.parent_id)==Embedding.id, "
                    "PipelineRunItem.parent_type=='embedding')",
        viewonly=False,
    )

    @classmethod
    def create(cls, session, parent_type, parent_id, model_name, embedding_vector, meta_json=None):
...
class PipelineRun(Base):
    ...
    models_json = Column(JSONB)
    env_json = Column(JSONB)

    started_at = Column(TIMESTAMP(timezone=True),server_default=func.timezone('UTC', func.now()),nullable=False)
