...
from configuration.config import MODEL_TRAINING_PARTS_MODEL_DIR    # canonical parts model dir
from seqeval.metrics import f1_score, precision_score, recall_score
from configuration.log_config import TrainingLogManager, maintain_training_logs, get_intent_ner_logger

# ===================== Logging =====================
log = get_intent_ner_logger("parts_ner")
log.setLevel(logging.INFO)
log.info("Starting Parts NER training script")

gpu_adapter = GPUTrainingAdapter()
log.info("[GPU] Adapter status: %s", gpu_adapter.describe())
...
log.info("Training with label set: %s", LABELS)
...
def prune_old_runs(base_dir: Path, keep: int = 5) -> None:
    ...
    for old in runs[keep:]:
        try:
            shutil.rmtree(old)
            log.info("[CLEANUP] Removed old run dir: %s", old)
        except Exception as e:
            log.warning("[CLEANUP] Could not remove %s: %s", old, e)
...
def save_best_artifacts(trainer: Trainer, tokenizer: AutoTokenizer, run_dir: Path) -> None:
    ...
    log.info("[SAVE] Best model + tokenizer saved to: %s", best_dir)
...
MODEL_DIR = resolve_latest_model(Path(MODEL_TRAINING_PARTS_MODEL_DIR ))  # .../best
...
def choose_dataset_size() -> Optional[int]:
    while True:
        ...
        print("Invalid or empty choice. Please enter 1, 2, or 3.\n")
...
class WeightedTrainer(Trainer):
    ...
    def compute_loss(
        ...
        **kwargs,  # <-- CRITICAL FIX (absorbs num_items_in_batch)
    ):
...
def main():
    ...
    log.info("Resolved base model dir: %s", MODEL_DIR)
    ...
    log.info("[TRAIN] Output run directory: %s", run_dir)
    ...
    training_args = TrainingArguments(
        ...
        fp16=gpu_adapter.local_cuda,
        dataloader_pin_memory=gpu_adapter.local_cuda,
    )
    ...
    with TrainingLogManager(run_dir=run_dir, to_console=False) as tlogm:
        ...
        trainer = gpu_adapter.wrap_trainer(WeightedTrainer(
            model=model,
            args=training_args,
