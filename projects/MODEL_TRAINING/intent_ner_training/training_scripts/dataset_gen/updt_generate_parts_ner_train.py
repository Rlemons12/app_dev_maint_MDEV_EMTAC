...
# --- DB config / models ---
from modules.configuration.config_env import TrainingDatabaseConfig
...
INTENT_SCHEMA = "intent_training"
metadata = MetaData(schema=INTENT_SCHEMA)
...
def ensure_training_data_table(engine):
    ddl = """
    CREATE TABLE IF NOT EXISTS intent_training.training_data (
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS uq_training_data_type_content_label
        ON intent_training.training_data (data_type, data_content, "label");
    """
...
def build_ner_samples(df: pd.DataFrame, colmap: Dict[str, str]) -> List[Dict]:
    """
    Build overlap-safe, validated NER training samples.

    Guarantees:
    - No overlapping spans
    - Accurate start/end offsets
    ...
    for row_idx, row in df.iterrows():
        ...
        # --------------------------------------------------
        # 1. Build utterance text (deterministic)
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        # 2. Overlap-safe entity extraction
        # --------------------------------------------------
        ents: List[Dict] = []
        cursor = 0
        ...
        def add_ent(value, label):
            ...
            if label not in PARTS_ENTITY_SCHEMA:
            ...
            cursor = end  # 🔐 advance cursor to prevent overlap
        ...
        for i in range(len(spans) - 1):
            if spans[i][1] > spans[i + 1][0]:
                warning_id(
                    f"Skipping row {row_idx}: overlapping spans detected: {spans}"
                )
                ents = []
                break
        ...
        # Ensure text slices match entity text exactly
        bad = False
        for e in ents:
            extracted = utter[e["start"]:e["end"]]
            if extracted != e["text"]:
                warning_id(
                    f"Span mismatch row {row_idx}: '{extracted}' != '{e['text']}'"
                )
                bad = True
                break
    ...
    logger.info("Built %d validated NER samples from loadsheet.", len(samples))
    return samples
...
def upsert_intents_into_training_data(
    ...
    """
    Insert intent texts into intent_training.training_data with data_type='intent'.
...
def main():
    ...
    with cfg.get_session() as session:
