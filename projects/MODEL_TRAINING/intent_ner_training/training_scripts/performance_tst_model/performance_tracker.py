...
class PerformanceTracker:
    ...
    def print_summary_report(self):
        """Print a concise summary report (schema-safe)."""
        report = self.generate_performance_report()
        ...
        print(f"Average Execution Time: {summary.get('average_execution_time_ms', 0.0):.1f}ms")
        print(f"Test Duration: {summary.get('test_duration_minutes', 0.0):.1f} minutes")
        ...
        for entity, metrics in report.get("entity_performance", {}).items():
            total = metrics.get("total_tests", metrics.get("total", 0))
            if total > 0:
                print(
                    f"{entity.upper():15} | "
                    f"Exact: {metrics.get('exact_accuracy', 0.0):.2%} "
                    f"Partial: {metrics.get('partial_accuracy', 0.0):.2%} "
        ...
        for pattern, metrics in report.get("pattern_performance", {}).items():
            total = metrics.get("total_tests", metrics.get("total", 0))
            success = metrics.get("successful", metrics.get("success", 0))

            if total > 0:
                rate = success / total if total else 0.0
                print(
                    f"{pattern:20} | Success: {rate:.2%} ({success}/{total})"
                )
        ...
        print(f"\nConfidence Analysis:")
        print("-" * 50)
        print(f"Average Confidence: {conf.get('average_confidence', 0.0):.3f}")
        print(f"Correct Predictions: {conf.get('correct_predictions_confidence', 0.0):.3f}")
        print(f"Incorrect Predictions: {conf.get('incorrect_predictions_confidence', 0.0):.3f}")

        thresh = conf.get("confidence_threshold_analysis", {})
        if "optimal_threshold" in thresh:
            print(
                f"Optimal Threshold: {thresh['optimal_threshold']:.1f} "
                f"(F1: {thresh.get('optimal_f1', 0.0):.3f})"
            )
