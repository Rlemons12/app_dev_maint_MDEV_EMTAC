...
from configuration.log_config import debug_id, info_id, error_id, warning_id, with_request_id, set_request_id
from configuration.config import MODEL_TRAINING_DRAWINGS_TRAIN_DATA_DIR, MODEL_TRAINING_DRAWINGS_MODEL_DIR
...
info_id(f"[GPU-ADAPTER] {gpu_adapter.describe()}", test_session_id)
...
try:
    ...
    from intent_ner_training.training_scripts.performance_tst_model.performance_tracker import (PerformanceTracker,
                                                                                                QueryResult,EntityMatch)
...
try:
    from configuration.config import MODEL_TRAINING_DRAWINGS_MODEL_DIR, MODEL_TRAINING_DRAWINGS_TRAIN_DATA_DIR

    info_id("Config imported successfully", test_session_id)
    debug_id(f"DRAWINGS_MODEL_DIR: {MODEL_TRAINING_DRAWINGS_MODEL_DIR}", test_session_id)
    debug_id(f"DRAWINGS_TRAIN_DATA_DIR: {MODEL_TRAINING_DRAWINGS_TRAIN_DATA_DIR}", test_session_id)
except ImportError as e:
...
class ComprehensiveDrawingsNERTester:
    ...
    def load_model(self):
        ...
        try:
            ...
            # Prepare model via GPU adapter
            model = gpu_adapter.prepare_model(model)

            # Resolve device index for pipeline
            if gpu_adapter.remote_available:
                # Remote GPU service → pipeline stays CPU-side
                pipeline_device = -1
            ...
            info_id(f"[GPU] Pipeline device set to {pipeline_device}", self.session_id)
...
def main():
    ...
    try:
        ...
        # Configuration
        data_path = MODEL_TRAINING_DRAWINGS_TRAIN_DATA_DIR
        model_path = MODEL_TRAINING_DRAWINGS_MODEL_DIR
