...
# ---------------------------------------------------
# MLflowLogManager: isolated MLflow tracking logger
# ---------------------------------------------------

class MLflowLogManager:
    """
    Dedicated MLflow logger.

    Characteristics:
    - Completely isolated from app + training logs
    - No request-id logic
    - Safe for long-running loops
    - File-based with optional console mirror
    - Designed for MLflow lifecycle + diagnostics
    """
    ...
    def __init__(
        ...
        self.run_name = run_name or "mlflow"
        self.level = level
        self.to_console = to_console
        self.rotate_mb = rotate_mb
        self.backups = backups
        ...
        # -------------------------------------------------
        base_mlflow_log_dir = Path(log_directory) / "mlflow"
        base_mlflow_backup_dir = Path(log_backup_directory) / "mlflow"

        base_mlflow_log_dir.mkdir(parents=True, exist_ok=True)
        base_mlflow_backup_dir.mkdir(parents=True, exist_ok=True)

        self.log_dir = Path(log_dir) if log_dir else base_mlflow_log_dir

        # -------------------------------------------------
        # Logger setup
        # -------------------------------------------------
        self.logger_name = f"ematac_mlflow.{self.run_name}"
        self._logger = logging.getLogger(self.logger_name)
    ...
    # -------------------------------------------------
    def _attach_handlers(self):
        log_path = self.log_dir / f"{self.run_name}.log"
        log_path.parent.mkdir(parents=True, exist_ok=True)

        # Avoid duplicate handlers on re-instantiation
        def _same_file(h):
            return isinstance(h, RotatingFileHandler) and getattr(
                h, "baseFilename", None
        ...
        if not any(_same_file(h) for h in self._logger.handlers):
            ...
            fh.setLevel(self.level)
            fh.setFormatter(logging.Formatter(
                "%(asctime)s - mlflow - %(levelname)s - %(message)s"
            ))
            self._logger.addHandler(fh)

        if self.to_console and not any(
    ...
    # Lifecycle helpers
    # -------------------------------------------------
    def log_run_start(self, experiment: str, run_name: str):
        self._logger.info(
    ...
    def log_param(self, key: str, value):
        self._logger.debug("PARAM | %s = %s", key, value)

    def log_metric(self, key: str, value, step: Optional[int] = None):
        if step is not None:
            self._logger.debug("METRIC | %s = %s | step=%s", key, value, step)
        else:
            self._logger.debug("METRIC | %s = %s", key, value)

    def log_artifact(self, path: str):
        self._logger.info("ARTIFACT | %s", path)

    def log_exception(self, exc: Exception):
        self._logger.exception("MLflow exception: %s", exc)
    ...
    def __exit__(self, exc_type, exc, tb):
        if exc:
            self._logger.exception("MLflow run failed")
        self.close()


def get_intent_ner_logger(name: str = "intent_ner") -> logging.Logger:
    """
    Intent + NER logger.
    Inherits handlers and configuration from emtac_logger.
    """
    logger = logging.getLogger(f"emtac_intent_ner.{name}")
    logger.setLevel(logging.DEBUG)
    logger.propagate = True  # inherit handlers from emtac_logger
    return logger
